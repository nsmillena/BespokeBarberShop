...
if (empty($unidade_id)) { header('Location: index_admin.php?ok=0&msg=' . urlencode(t('admin.err_admin_no_unit_scales'))); exit; }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && !$tablesReady){
  header('Location: escalas.php?barbeiro='.(int)($_POST['barbeiro']??0).'&ok=0&msg=' . urlencode(t('admin.migration_needed_scales')));
  exit;
}

// POST (somente com tabelas prontas)
if ($_SERVER['REQUEST_METHOD']==='POST' && $tablesReady){
  $t = $_POST['csrf_token'] ?? ''; if (!hash_equals($_SESSION['csrf_token'],$t)) { header('Location: escalas.php?ok=0&msg=' . urlencode(t('user.security_fail'))); exit; }
  $acao = $_POST['acao'] ?? '';
  $bId = (int)($_POST['barbeiro'] ?? 0);
  $chk = $conn->prepare("SELECT 1 FROM Barbeiro WHERE idBarbeiro=? AND Unidade_idUnidade=?");
  $chk->bind_param('ii', $bId, $unidade_id); $chk->execute(); $ok = $chk->get_result()->num_rows>0; $chk->close();
    if(!$ok){ header('Location: escalas.php?ok=0&msg=' . urlencode(t('admin.invalid_barber'))); exit; }
  ...
  if ($acao==='add_folga'){
    ...
    $dFim = $fim ? DateTime::createFromFormat('Y-m-d',$fim) : null;
      if (!$dIni || ($fim && !$dFim)) { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.invalid_dates'))); exit; }
      if ($dFim && $dFim < $dIni) { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.end_before_start_date'))); exit; }
      if ($weekday < 1 || $weekday > 7) { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.invalid_weekday'))); exit; }
    ...
    $q->bind_param('iiss', $bId, $weekday, $iniStr, $fimStr); $q->execute(); $q->store_result();
      if ($q->num_rows>0){ $q->close(); header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.weekly_off_overlap'))); exit; }
    $q->close();
    ...
    $ins->bind_param('iiss', $bId, $weekday, $iniStr, $fimStr);
      if ($ins->execute()) { header('Location: escalas.php?barbeiro='.$bId.'&ok=1&msg=' . urlencode(t('admin.weekly_off_created'))); exit; }
      else { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.weekly_off_create_fail'))); exit; }
  }
  if ($acao==='del_folga'){
    $id = (int)($_POST['id'] ?? 0);
    $del = $conn->prepare("DELETE FROM FolgaSemanal WHERE idFolga=? AND Barbeiro_idBarbeiro=?");
    $del->bind_param('ii', $id, $bId);
      if ($del->execute()) { header('Location: escalas.php?barbeiro='.$bId.'&ok=1&msg=' . urlencode(t('admin.weekly_off_removed'))); exit; }
      else { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.weekly_off_remove_fail'))); exit; }
  }
  if ($acao==='add_ferias'){
    $inicio = $_POST['inicio'] ?? '';
    $fim = $_POST['fim'] ?? '';
    ...
    $dIni = DateTime::createFromFormat('Y-m-d',$inicio); $dFim = DateTime::createFromFormat('Y-m-d',$fim);
      if (!$dIni || !$dFim || $dFim < $dIni) { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.vacation_invalid_period'))); exit; }
    ...
    $q->bind_param('iss', $bId, $iniStr, $fimStr); $q->execute(); $q->store_result();
      if ($q->num_rows>0){ $q->close(); header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.vacation_overlap'))); exit; }
    $q->close();
    ...
    $ins->bind_param('isss', $bId, $iniStr, $fimStr, $motivo);
      if ($ins->execute()) { header('Location: escalas.php?barbeiro='.$bId.'&ok=1&msg=' . urlencode(t('admin.vacation_created'))); exit; }
      else { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.vacation_create_fail'))); exit; }
  }
  if ($acao==='del_ferias'){
    $id = (int)($_POST['id'] ?? 0);
    $del = $conn->prepare("DELETE FROM FeriasBarbeiro WHERE idFerias=? AND Barbeiro_idBarbeiro=?");
    $del->bind_param('ii', $id, $bId);
      if ($del->execute()) { header('Location: escalas.php?barbeiro='.$bId.'&ok=1&msg=' . urlencode(t('admin.vacation_removed'))); exit; }
      else { header('Location: escalas.php?barbeiro='.$bId.'&ok=0&msg=' . urlencode(t('admin.vacation_remove_fail'))); exit; }
  }
}
...
<head>
  ...
  <title><?= t('admin.scales_title') ?> | Admin</title>
...
<div class="container py-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="dashboard-section-title mb-0"><i class="bi bi-calendar3"></i> <?= t('admin.scales_title') ?></h2>
  ...
  <?php if (!$tablesReady): ?>
    <div class="alert alert-warning"><?= t('admin.migration_needed_scales') ?></div>
  <?php endif; ?>
  ...
  <div class="row g-4">
    <div class="col-12 col-lg-6">
      <div class="dashboard-card p-3 h-100">
        <h5 class="mb-3"><i class="bi bi-calendar-week"></i> <?= t('admin.weekly_off') ?></h5>
        <?php if ($tablesReady): ?>
        <form method="POST" class="row g-2 align-items-end mb-3">
          ...
          <div class="col-12 col-sm-4">
            <label class="form-label"><?= t('admin.weekday') ?></label>
          ...
          <div class="col-6 col-sm-4">
            ...
            <input type="date" name="inicio" class="form-control" placeholder="<?= bb_is_en() ? 'YYYY-MM-DD' : 'AAAA-MM-DD' ?>" required>
          </div>
          <div class="col-6 col-sm-4">
            <label class="form-label"><?= t('admin.end') ?> (<?= t('admin.optional') ?>)</label>
            <input type="date" name="fim" class="form-control" placeholder="<?= bb_is_en() ? 'YYYY-MM-DD' : 'AAAA-MM-DD' ?>">
          </div>
          <div class="col-12">
            <button type="submit" class="btn btn-primary"><i class="bi bi-plus-circle"></i> <?= t('admin.add_weekly_off') ?></button>
          </div>
        </form>
        <?php endif; ?>
        ...
        <div class="table-responsive">
          <table class="table table-dark table-striped align-middle dashboard-table">
            <thead><tr><th><?= t('admin.day') ?></th><th><?= t('admin.data') ?></th><th><?= t('admin.end') ?></th><th><?= t('common.actions') ?></th></tr></thead>
            <tbody>
              <?php if ($tablesReady && count($folgas)>0): foreach($folgas as $f): ?>
                <tr>
                  <td><?= htmlspecialchars($weekdayNames[(int)$f['weekday']] ?? (string)$f['weekday']) ?></td>
                  <td><?= bb_format_date($f['inicio']) ?></td>
                  <td><?= $f['fim'] ? bb_format_date($f['fim']) : '—' ?></td>
                  <td>
                    <form method="POST" onsubmit="return confirm('<?= t('admin.confirm_remove_weekly_off') ?>');" style="display:inline-block;">
                      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
              ...
              <?php endforeach; else: ?>
                <tr><td colspan="4" class="text-center text-muted"><?= t('admin.no_weekly_off') ?></td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
    ...
    <div class="col-12 col-lg-6">
      <div class="dashboard-card p-3 h-100">
        <h5 class="mb-3"><i class="bi bi-airplane"></i> <?= t('admin.vacation') ?></h5>
        <?php if ($tablesReady): ?>
        <form method="POST" class="row g-2 align-items-end mb-3">
          <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
          ...
          <div class="col-6">
            ...
            <input type="date" name="fim" class="form-control" placeholder="<?= bb_is_en() ? 'YYYY-MM-DD' : 'AAAA-MM-DD' ?>" required>
          </div>
          <div class="col-12">
            <label class="form-label"><?= t('admin.reason_optional') ?></label>
            <input type="text" name="motivo" class="form-control" maxlength="150" placeholder="<?= t('admin.reason_placeholder') ?>">
          </div>
          <div class="col-12">
            <button type="submit" class="btn btn-primary"><i class="bi bi-plus-circle"></i> <?= t('admin.add_vacation') ?></button>
          </div>
        </form>
        <?php endif; ?>
        ...
        <div class="table-responsive">
          <table class="table table-dark table-striped align-middle dashboard-table">
            <thead><tr><th><?= t('admin.data') ?></th><th><?= t('admin.end') ?></th><th><?= t('admin.description') ?></th><th><?= t('common.actions') ?></th></tr></thead>
            <tbody>
              <?php if ($tablesReady && count($ferias)>0): foreach($ferias as $f): ?>
                <tr>
                  <td><?= bb_format_date($f['inicio']) ?></td>
                  <td><?= bb_format_date($f['fim']) ?></td>
                  <td><?= htmlspecialchars($f['motivo'] ?? '') ?></td>
                  <td>
                    <form method="POST" onsubmit="return confirm('<?= t('admin.confirm_remove_vacation') ?>');" style="display:inline-block;">
                      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
              ...
              <?php endforeach; else: ?>
                <tr><td colspan="4" class="text-center text-muted"><?= t('admin.no_vacation') ?></td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
